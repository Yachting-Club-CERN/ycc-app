...
const getToJsonOrValue = (value: unknown): unknown => {
  // typeof null === 'object', so go on if this value is really an object but not
  // one of the weird builtin objects.
  return value &&
    typeof value === "object" &&
    "toJSONObject" in value &&
    typeof value.toJSONObject === "function"
    ? (value as { toJSONObject: () => unknown }).toJSONObject()
    : value;
};
...
export function decycle(object: unknown): unknown {
  ...
  return (function derez(value: unknown, path: string): unknown {
    ...
    const _value: unknown = getToJsonOrValue(value);
